...
export function findQuestion(
    ...
    const foundID = questions.find((q: Question): boolean => q.id === id);
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce(
        (sum: number, q: Question) => (q.published ? (sum += q.points) : sum),
        0,
    );
}
...
export function renameQuestionById(
    ...
    return questions.map((q: Question): Question => {
        return { ...q, name: q.id === targetId ? newName : q.name };
    });
}
...
export function duplicateQuestionInArray(
    ...
    const targetQuestionInd: number = questions.findIndex(
        (q: Question): boolean => q.id === targetId,
    );

    return questions.splice(targetQuestionInd, 0, targetQuestion);
}
