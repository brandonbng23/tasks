...
export function stringsToIntegers(numbers: string[]): number[] {
    return numbers.map((number: string): number => (+number ? +number : 0));
}
...
export function countShortWords(words: string[]): number {
    return words.reduce(
        (counter: number, word: string) =>
            word.length < 4 ? (counter += 1) : counter,
        0,
    );
}
...
export function makeMath(addends: number[]): string {
    const sum = addends.reduce(
        (sum: number, addend: number) => (sum += addend),
        0,
    );
    const expression = addends.join("+");
    return addends.length === 0 ? "0=0" : sum + "=" + expression;
}
...
export function injectPositive(values: number[]): number[] {
    ...
    const sum =
        values.some((value: number): boolean => value < 0) ?
            sigma.reduce((sum: number, value: number) => sum + value, 0)
